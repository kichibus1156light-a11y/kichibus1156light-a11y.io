...
const Stage = (function () {
  ...
  function init(canvas) {
    isMobile = Math.min(window.innerWidth, window.innerHeight) < 620 ||
               /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent);
    quality = isMobile ? 0.6 : 1;
  ...
  function buildColony() {
    ...
    // G3ガスFX（緑の霧）
    const gasCount = isMobile ? 120 : 260;
  ...
  function buildFleets() {
    FLEETS.forEach(function (f) {
      ...
      const ships = [];
      for (let i = 0; i < Math.max(3, Math.round(f.count * (isMobile ? 0.7 : 1))); i++) {
